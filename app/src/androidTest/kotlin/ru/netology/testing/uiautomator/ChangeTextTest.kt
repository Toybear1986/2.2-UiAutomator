...
class ChangeTextTest {
    ...
    private val textForAnotherActivity = "TestNewActivity"
...
//    }
    ...
    @Test
    fun textNoChangesIfInputTextIsEmpty() {
        val packageName = MODEL_PACKAGE
        waitForPackage(packageName)

        val textBeforeChanges = device.findObject(By.res(packageName, "textToBeChanged"))
            .text
        ...
        val result = device.findObject(By.res(packageName, "textToBeChanged")).text
        assertEquals(result, textBeforeChanges)
    }

    @Test
    fun textProperlyDisplayedInNewActivity() {
        val packageName = MODEL_PACKAGE
        waitForPackage(packageName)

        device.findObject(By.res(packageName, "userInput")).text = textForAnotherActivity
        device.findObject(By.res(packageName, "buttonActivity"))
            .clickAndWait(Until.newWindow(), 3000)

        val result = device.findObject(By.res(packageName, "text")).text
        assertEquals(result, textForAnotherActivity)
    }
